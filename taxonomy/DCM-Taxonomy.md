...
| **Placement Policy** | Typed Policy (the 8th). Output: placement constraints — affinity / anti-affinity / spread / co-locate / prefer / pin (+ weights) over **abstract `Topology` kinds** (never provider-native ids). Declarative; the **Placement Engine** evaluates it. Consumes UDLM `Topology` + capacity/utilization + provider `topology_capability`. See [ADR-019](../architecture/adr/019-placement-policy.md). |
...
| Term | Definition |
|------|-----------|
| **Request Orchestrator** | The event bus. Routes lifecycle events to **all subscribers** — the Policy Engine **and** any other component that consumes them (e.g. cost management, drift reconciliation). Consumer-neutral, never coupled to one engine. Contains no pipeline logic — Policies define all behavior. |
...
| Term | Definition |
|------|-----------|
| **Red Hat Developer Hub (RHDH)** | Red Hat's enterprise distribution of Backstage. *One* deployment target for the DCM consumer GUI (not required — the GUI is realization-neutral). Provides pre-built auth (RHSSO/Keycloak), RBAC plugin, ArgoCD/Tekton/AAP integrations, and the PatternFly design system. DCM's RHDH plugins ship via **supported (static) plugin builds**, not runtime dynamic plugins (the Backstage Dynamic Plugin mechanism was rejected upstream). |
...
| Term | Definition |
|------|-----------|
| **Unified Shell** | One DCM web application with **role-gated surfaces** (views of one app, not separate apps): **Consumer Portal** (developers/owners/tenant-admins; wraps the Consumer API), **Admin Panel** (platform roles; wraps the Admin API), **Provider Management** (provider_owner), **Flow GUI** (policy_owner/sre). One login, one session; navigation adapts to the actor's privilege. *Per-surface features and the `GUI-001–GUI-010` capabilities are specified in the GUI specs (`docs/specifications/dcm-{consumer,admin,provider,flow}-gui-spec.md`), not enumerated in the taxonomy (right-altitude: the taxonomy names the concept; the spec carries the detail).* |
...
| **RTO / RPO** | **Standard DR objectives — adopted by reference, not redefined.** Only the DCM-specific *targets* are normative: **RTO** profile-governed PT1M–PT5M (component) / PT5M–PT30M (full control-plane loss); **RPO** GitOps stores 0, Realized Store PT1M (fsi/sovereign)–PT5M (standard), Audit Store PT1M–PT15M. |
...
| Term | Definition |
|------|-----------|
| **Liveness / Readiness (`/livez`, `/readyz`)** | The **standard Kubernetes probe convention — adopted by reference**, not redefined. DCM-specific only: `livez` PT5S max / no external calls / unauthenticated (failure → restart); `readyz` validates Session Store, Audit Store, Policy Engine, Message Bus, Auth Provider (failure → out of LB). |
